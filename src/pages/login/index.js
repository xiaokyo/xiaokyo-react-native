...
export default props => {
  ...
  const {params, handleChange, submit, error, loginLoading} = useLoginForm({
    navigate,
  });
  ...
  return (
    <Wrapper>
      ...
      <TextInput
        ...
        placeholder="用户名/邮箱"
        onChangeText={value => handleChange({key: 'username', value})}
      />
      <Hr />
      <TextInput
        maxLength={20}
      ...
      <Hr />
      <TouchableOpacity
        activeOpacity={error ? 1 : 0.4}
        onPress={() => !error && submit()}>
        <LoginButton {...{error}}>
          <LinearGradient
            colors={['#ecccac', '#e3a86c']}
      ...
      <Footer>
        ...
        <FootButton onPress={() => navigate('Register')}>
...
const useLoginForm = ({navigate}) => {
  ...
  const [login, {loading: loginLoading}] = useMutation(TODO_LOGIN);
